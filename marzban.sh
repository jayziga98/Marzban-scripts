...
install_marzban_script() {
    FETCH_REPO="jayziga98/Marzban-scripts"
...
install_marzban() {
    ...
    # Fetch releases
    FILES_URL_PREFIX="https://raw.githubusercontent.com/jayziga98/Marzban/master"
    ...
    if [ "$marzban_version" == "latest" ]; then
        sed -i "s|image: jayziga98/marzban:.*|image: jayziga98/marzban:latest|g" "$docker_file_path"
    else
        sed -i "s|image: jayziga98/marzban:.*|image: jayziga98/marzban:${marzban_version}|g" "$docker_file_path"
    fi
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/jayziga98/Marzban/releases"
